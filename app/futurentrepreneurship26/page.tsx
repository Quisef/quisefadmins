  ...
  const testimonials = [
    {
      name: "Chioma Adeleke",
      batch: "2025 Batch A",
      quote: "This program turned my business idea into a funded reality. The mentorship was world-class.",
      business: "Tech Solutions Ltd"
    },
    {
      name: "Ibrahim Mohammed",
      batch: "2025 Batch B",
      quote: "The seed funding and guidance helped launch my agribusiness successfully.",
      business: "Green Farms Nigeria"
    },
    {
      name: "Blessing Okafor",
      batch: "2025 Batch A",
      quote: "The network and connections opened doors I never imagined possible.",
          ...
          {testimonials.map((_, i) => (
            <button
              ...
              className={`h-3 rounded-full transition-all ${
                i === index ? 'w-10 bg-white shadow' : 'w-3 bg-white/50'
              }`}
...
function CategoryCard({ category, onSelect }: { category: Category; onSelect: () => void }) {
  return (
    <div className={`
      ${category.color} border-2 rounded-3xl p-7 transition-all duration-400
      hover:shadow-2xl hover:scale-[1.03] hover:-translate-y-2 group
    `}>
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <span className={`inline-block ${category.badge} text-white text-xs px-4 py-1.5 rounded-full font-semibold tracking-wide`}>
            {category.type}
          </span>
        </div>
        <div className="text-right">
          <p className="text-1xl font-extrabold text-gray-900 tracking-tight">{category.price}</p>
      ...
      <button
        onClick={onSelect}
        className="w-full bg-gradient-to-r from-gray-900 to-gray-800 text-white py-4 rounded-xl font-semibold hover:from-gray-800 hover:to-black transition-all shadow-lg hover:shadow-xl group-hover:scale-[1.02] flex items-center justify-center gap-2"
      >
...
const categories = [
  {
    ...
    benefits: [
      ...
      'Business plan competition → Grant eligible',
      'Lifetime Alumni Network access',
      'Graduation ceremony (transport + accommodation)',
    ],
    notIncluded: [],
  },
  {
    ...
    benefits: [
      'Full access to training modules',
      'Dedicated mentorship program',
      'Business plan competition → Seed funding',
      'Lifetime Alumni Network access',
      'Graduation ceremony attendance',
    ],
    notIncluded: [
      'Transportation to graduation ceremony',
  ...
  },
  {
    ...
    benefits: [
      ...
      'Graduation ceremony attendance',
      'Business plan competition (performance-based)',
    ],
    notIncluded: [
      'Dedicated mentorship',
      'Transportation & accommodation for graduation',
    ],
  },
  {
    id: 'self-funded',
...
export default function FuturenTrepeneurship() {
  ...
  const [formData, setFormData] = useState({
    fullName: '', email: '', phone: '', stateCode: '', areaOfInterest: '', category: ''
  });
  ...
  const validateForm = () => {
    ...
    if (!formData.email.includes('@') || !formData.email.includes('.')) errs.email = "Valid email required";
    if (!formData.phone.match(/^\+?\d{9,15}$/)) errs.phone = "Valid phone number required";
    if (!formData.stateCode.trim()) errs.stateCode = "NYSC state code required";
    if (!formData.areaOfInterest.trim()) errs.areaOfInterest = "Area Of Interest required";
    setErrors(errs);
  ...
  const reset = () => {
    ...
    setFormData({ fullName:'', email:'', phone:'', stateCode:'', areaOfInterest:'', category:'' });
    setErrors({});
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50/60 flex items-center justify-center p-6">
        <div className="max-w-2xl w-full bg-white rounded-3xl shadow-2xl p-10 text-center border border-gray-100">
          ...
          <p className="text-sm text-gray-500 mb-8">
            ...
            Keep your ID safe for reference.
          </p>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl p-8 md:p-10 border border-gray-100/70">
            ...
            <form onSubmit={handleSubmit} className="space-y-7">
              {(['fullName','email','phone','stateCode','areaOfInterest'] as const).map(field => (
                <div key={field}>
                  ...
                  <input
                    ...
                    className={`
                      w-full px-5 py-4 border rounded-xl transition
                      focus:ring-2 focus:outline-none
                      ${errors[field] ? 'border-red-400 focus:ring-red-200' : 'border-gray-300 focus:ring-emerald-300 focus:border-emerald-500'}
                    `}
                    placeholder={
                      field === 'stateCode' ? 'e.g. AD/26A/12345' :
                      field === 'areaOfInterest' ? 'e.g. Agriculture, Architecture,IT,Marketing,Media & Content Creation, Trading etc' : ''
                    }
                  />
          ...
          <div className="space-y-5">
            {[
              ...
              <details key={i} className="group border border-gray-200 rounded-xl">
                <summary className="flex justify-between items-center p-6 cursor-pointer font-semibold text-lg text-gray-800 hover:text-emerald-700 transition">
                  {item.q}
